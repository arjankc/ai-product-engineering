...
app.post('/query', async (req, res) => {
  ...
  try {
    const response = await ai.models.generateContent({
      ...
      contents: userInput,
      config: { systemInstruction: 'You are Kathmandu Heritage Guide. Prefer domain-specific answers and clearly state uncertainty.' },
    });
...
app.listen(PORT, () => console.log(`Kathmandu Heritage Guide (Phase 2) → http://localhost:${PORT}`));
