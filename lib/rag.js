...
export async function ragQuery(ai, userQuestion, media = {}) {
  ...
  const response = await ai.models.generateContent({
    model: MODEL,
    contents,
    config: { systemInstruction: 'You are Automated Expense & Tax Analyst. Answer only from the supplied vault context, cite source filenames inline, distinguish mock data, and say when evidence is insufficient.' },
  });
