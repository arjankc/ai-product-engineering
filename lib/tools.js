...
/* Small fixed planning table; not a live settlement quote. */
export function convertCurrency(amount, from, to) {
  const ratesPerUsd = { USD: 1, NPR: 133, EUR: 0.92, INR: 83 };
  const source = String(from ?? '').toUpperCase();
  const target = String(to ?? '').toUpperCase();
  const value = Number(amount);
  if (!Number.isFinite(value) || !ratesPerUsd[source] || !ratesPerUsd[target]) {
    return { error: 'Supported currencies are NPR, USD, EUR, and INR.' };
  }
  const converted = (value / ratesPerUsd[source]) * ratesPerUsd[target];
  return { amount: value, from: source, to: target, converted: Number(converted.toFixed(2)) };
}
...
export async function executeTool(name, args, ctx = {}) {
  ...
  if (name === 'convert_currency') {
    return JSON.stringify(convertCurrency(args.amount, args.from, args.to));
  }
...
export const searchKnowledgeBaseTool = {
  ...
  description:
    'Search the trip planning vault (Obsidian vault embeddings) for relevant notes.',
  parametersJsonSchema: {
    type: 'object',
    properties: {
      query: {
...
export const domainTool = {
  ...
  description: 'Estimate a currency conversion using the classroom FX table.',
  parametersJsonSchema: {
    type: 'object',
    properties: {
      amount: { type: 'number', description: 'Amount to convert' },
      from: { type: 'string', description: 'Source currency: NPR, USD, EUR, or INR' },
      to: { type: 'string', description: 'Target currency: NPR, USD, EUR, or INR' },
    },
