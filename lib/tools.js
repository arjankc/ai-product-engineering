...
/* Mock local device reachability for the classroom demo. */
export function pingDevice(ipAddress) {
  const ip = String(ipAddress ?? '').trim();
  const statuses = {
    '192.168.1.1': 'online',
    '192.168.1.50': 'offline',
  };
  return { ip_address: ip, status: statuses[ip] || 'unknown' };
}
...
export async function executeTool(name, args, ctx = {}) {
  ...
  if (name === 'ping_device') {
    return JSON.stringify(pingDevice(args.ip_address));
  }
...
export const searchKnowledgeBaseTool = {
  ...
  description:
    'Search the local hardware support vault (Obsidian vault embeddings) for relevant notes.',
  parametersJsonSchema: {
    type: 'object',
    properties: {
      query: {
...
export const domainTool = {
  name: 'ping_device',
  description: 'Return mock reachability for a local IPv4 device.',
  parametersJsonSchema: {
    type: 'object',
    properties: {
      ip_address: { type: 'string', description: 'Local IPv4 address to ping' },
    },
    required: ['ip_address'],
  },
};
