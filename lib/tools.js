...
/* Create a real timestamp payload without inventing an external timer URL. */
export function generateWorkoutTimer(restSeconds) {
  const seconds = Math.max(0, Math.floor(Number(restSeconds)));
  if (!Number.isFinite(seconds)) return { error: 'rest_seconds must be a number.' };
...
export async function executeTool(name, args, ctx = {}) {
  ...
  if (name === 'generate_workout_timer') {
    return JSON.stringify(generateWorkoutTimer(args.rest_seconds));
  }
...
export const searchKnowledgeBaseTool = {
  ...
  description:
    'Search the fitness and rehabilitation vault (Obsidian vault embeddings) for relevant notes.',
  parametersJsonSchema: {
    type: 'object',
    properties: {
      query: {
...
export const domainTool = {
  name: 'generate_workout_timer',
  description: 'Generate start and end timestamps for a workout rest period.',
  parametersJsonSchema: {
    type: 'object',
    properties: {
      rest_seconds: { type: 'number', description: 'Non-negative rest duration in seconds' },
    },
    required: ['rest_seconds'],
  },
};
