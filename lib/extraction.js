...
/**
 * Vision-capable model used for image extraction. Override with GEMINI_MODEL when
 * one model's quota is exhausted, since free-tier limits are per model per project.
 * @type {string}
 */
export const EXTRACTION_MODEL = process.env.GEMINI_MODEL || 'gemini-2.5-flash';
...
export function isRetryableError(err) {
  ...
  // A per-day quota reset is hours away, so backing off for seconds is pointless.
  if (/PerDay|per day|quotaValue/i.test(text)) return false;
