...
app.post('/query', async (req, res) => {
  ...
  try {
    const response = await ai.models.generateContent({
      ...
      contents: userInput,
      config: { systemInstruction: 'You are Plant Care Assistant. Prefer domain-specific answers and clearly state uncertainty.' },
    });
...
app.listen(PORT, () => console.log(`Plant Care Assistant (Phase 2) → http://localhost:${PORT}`));
