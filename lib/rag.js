...
export async function ragQuery(ai, userQuestion, media = {}) {
  ...
  const systemInstruction = [
    'You are the Smart Pantry & Recipe Architect.',
    'Answer ONLY using the provided CONTEXT from the household pantry vault.',
    'If the context is insufficient, say you do not have that information in the household notes.',
  ...
  if (media.base64Image) {
    ...
    contents.push(
      `${prompt}\n\nAlso interpret the attached fridge/pantry image using only household rules from CONTEXT.`,
    );
  } else {
