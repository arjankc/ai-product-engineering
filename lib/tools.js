...
export function roll_dice(notation) {
  const match = String(notation ?? '').trim().match(/^(\d*)d(\d+)([+-]\d+)?$/i);
  if (!match) throw new Error('Use NdM+K notation, for example 2d6+3.');
  const count = Number(match[1] || 1);
  const sides = Number(match[2]);
  const modifier = Number(match[3] || 0);
  if (count < 1 || count > 100 || sides < 2 || sides > 1000) throw new Error('Dice limits are 1-100 dice with 2-1000 sides.');
  const rolls = Array.from({ length: count }, () => 1 + Math.floor(Math.random() * sides));
  return { notation: `${count}d${sides}${modifier >= 0 ? '+' : ''}${modifier}`, rolls, modifier, total: rolls.reduce((a, b) => a + b, 0) + modifier };
}
...
export async function executeTool(name, args, context = {}) {
  ...
  if (name === 'roll_dice') {
    try { return JSON.stringify(roll_dice(args.notation)); } catch (error) { return `Error: ${error.message}`; }
  }
...
export const domainTool = {
  name: 'roll_dice',
  description: 'Parse and roll standard NdM+K dice notation with bounded inputs.',
  parametersJsonSchema: { type: 'object', properties: { notation: { type: 'string', description: 'Dice notation such as 2d6+3 or d20-1' } }, required: ['notation'] },
};
