...
app.post('/query', async (req, res) => {
  ...
  try {
    const response = await ai.models.generateContent({
      ...
      contents: userInput,
      config: { systemInstruction: 'You are Expense Analyst. Prefer domain-specific answers and clearly state uncertainty.' },
    });
...
app.listen(PORT, () => console.log(`Expense Analyst (Phase 2) → http://localhost:${PORT}`));
