...
export function check_local_weather(city) {
  const key = String(city ?? '').trim().toLowerCase();
  const weather = {
    kathmandu: { city: 'Kathmandu', temperature_c: 24, humidity_percent: 68, condition: 'Cloudy', source: 'mock' },
    pokhara: { city: 'Pokhara', temperature_c: 27, humidity_percent: 82, condition: 'Light rain', source: 'mock' },
  };
  if (!weather[key]) return { error: 'Mock weather is available only for Kathmandu and Pokhara.' };
  return weather[key];
}
...
export async function executeTool(name, args, context = {}) {
  ...
  if (name === 'check_local_weather') {
    try { return JSON.stringify(check_local_weather(args.city)); } catch (error) { return `Error: ${error.message}`; }
  }
...
export const domainTool = {
  ...
  description: 'Return mock local growing conditions for Kathmandu or Pokhara.',
  parametersJsonSchema: { type: 'object', properties: { city: { type: 'string', description: 'Kathmandu or Pokhara' } }, required: ['city'] },
};
